...
cDNA_transcript_count <- as.data.frame(read.csv(paste0("/media/kilian/OS/Melani_THP1_August_2023/cDNA_ANALYSIS/stringtie_output_cDNA_specific_RMSK/transcript_count_matrix.csv"), sep = ","))
...
#Save dataframe
write.xlsx(cDNA_transcript_count_filter_top, paste0('/media/kilian/OS/Melani_THP1_August_2023/cDNA_ANALYSIS/stringtie_output_cDNA_specific_RMSK/cDNA_transcript_top.xlsx'))
...
dir.create(paste('plots/barplots/RMSK'))
...
for (i in goi) { 
  ...
  pdf(file = paste0('plots/barplots/RMSK/', i,'.pdf'), pointsize = 10, width = 5, height= 10)
  print(barplot)
  dev.off()
}
...
rmsk_repeats <- as.data.frame(read.csv(paste0("/media/kilian/OS/Melani_THP1_August_2023/reference/GRCh38_GENCODE_rmsk_TE.gtf"), sep = "\t", header = T))
rmsk_repeats <- rmsk_repeats[,-c(2:3,7:8)]

colnames(rmsk_repeats) <-  c('Chromosome', 'Start', 'End', 'Length', 'Name')
rmsk_repeats_edit<- cSplit(rmsk_repeats,'Name', ';')

colnames(rmsk_repeats_edit) <-  c('Chromosome', 'Start', 'End', 'Length', 'TE_subfamily','TE_unique_ID','TE_family','TE_superfamily')
rmsk_repeats_edit$TE_subfamily <- gsub('gene_id ','',rmsk_repeats_edit$TE_subfamily)
rmsk_repeats_edit$TE_unique_ID <- gsub('transcript_id ','',rmsk_repeats_edit$TE_unique_ID)
rmsk_repeats_edit$TE_family <- gsub('family_id ','',rmsk_repeats_edit$TE_family)
rmsk_repeats_edit$TE_superfamily <- gsub('class_id ','',rmsk_repeats_edit$TE_superfamily)

rmsk_repeats_edit$TE_unique_ID <- gsub('\\/','', rmsk_repeats_edit$TE_unique_ID)
...
colnames(cDNA_transcript_count_filter_top) <-  c('TE_unique_ID', colnames_cDNA_transcript_count_filter_top)

merged_TE_final <- dplyr::left_join(cDNA_transcript_count_filter_top, rmsk_repeats_edit, 'TE_unique_ID')

write.xlsx(merged_TE_final, paste('results/TE_filtered_Log2_counts.xlsx'))
...
###SELECT LABELS
select_UP <- merged_TE_final %>% arrange(desc(LOG2_FC)) %>% head(10) %>% dplyr::pull('TE_unique_ID')
select_DOWN <- merged_TE_final %>% arrange(LOG2_FC) %>% head(10) %>% dplyr::pull('TE_unique_ID')


#Select data of labels
select_UP_data <- merged_TE_final %>% filter(TE_unique_ID %in% c(select_UP))
select_DOWN_data <- merged_TE_final %>% filter(TE_unique_ID %in% c(select_DOWN))

#Select dimensions
col_max <- max(merged_TE_final$LOG2_FC)
volcano

scatter_volcano <- ggplot(merged_TE_final, aes(x= LOG2_FC, y= SUM, label = TE_unique_ID, size = SUM)) +
  geom_point(data = merged_TE_final,aes(size = SUM),  shape = 21, fill = 'grey') +
  ...
  scale_fill_gradient2(limits=c(-10, 10), low="navyblue", mid="whitesmoke", high = "firebrick", na.value = 'navy') +
  theme_scatter +
  #xlim(-15,15)+
  #ylim(0,9)+
...
Volcano_prefix
ggsave(paste0('plots/volcanos/','RMSK_Stringtie_output', '.pdf'),
...
merged_TE_final$UP_or_DOWN[merged_TE_final$LOG2_FC>0] <- 'UP'
merged_TE_final$UP_or_DOWN[merged_TE_final$LOG2_FC<0] <- 'DOWN'

donut_prefix <- 'TE_overall_change'
donut_data <- merged_TE_final
...
ggsave(paste0('plots/donut/',donut_prefix, '.pdf'),
       plot = piechart,
       device = NULL,
       path = NULL,
       #scale = 1,
...
#Family level 
donut_data_UP <- merged_TE_final %>% filter(UP_or_DOWN == 'UP')
donut_data_DOWN <-  merged_TE_final %>% filter(UP_or_DOWN == 'DOWN')

donut_prefix <- 'donut_data_UP'
donut_TEclass <- 'Family'
donut_data <- donut_data_UP

hsize <- 4

donut_data <- donut_data %>% 
  group_by(TE_family) %>% # Variable to be transformed
  dplyr::count() %>% 
  ungroup() %>% 
  mutate(perc = `n` / sum(`n`)) %>% 
  arrange(perc) %>%
...
# Basic Donut of TE elements
piechart <- ggplot(donut_data, aes(x=hsize, y= perc, fill=TE_family, label = labels)) +
  geom_bar(stat="identity", width=1) +
  coord_polar("y", start=0) + 
  geom_label(aes(label = labels),
             position = position_stack(vjust = 0.5),
...
ggsave(paste0('plots/donut/',donut_prefix, '_',donut_TEclass, '.pdf'),
       plot = piechart,
       device = NULL,
       path = NULL,
       #scale = 1,
